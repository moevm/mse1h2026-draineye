...
class LocalDamageModelDataSource {
  ...
  // одно изображение -> вектор вероятностей длины 3
  Future<List<double>> _runSingle(String path) async {
    ...
    // getInputTensor(0).data — unmodifiable (asUnmodifiableView в tflite_flutter);
    // пишем в свой Float32List и копируем через setter .data
  ...
  /// По каждому фото — вектор [corrosion, crack, no_damage]. Итог: по индексам
  /// среднее арифметическое, затем класс с максимальным средним значением.
  Future<ModelInferenceResult> runAverageProbabilities(List<String> photoPaths) async {
    if (photoPaths.isEmpty) {
      throw ArgumentError('photoPaths пуст');
    }
    if (kIsWeb) {
      throw UnsupportedError('TFLite на web не поддержан');
    }
